...
//        item.bringLocationAppsToFront()
    ...
    private func updateAlarmsIfNeeded() {
        if let events = Controllers.dataModel.dataModel?.events,
            let updatedEvents = self.updateAlarms(forItems: events)  {
            Controllers.dataModel.update(someEvents: updatedEvents)
            self.logger.log("Updated alarms for \(updatedEvents.count) events")
        }

        if  let reminders = Controllers.dataModel.dataModel?.reminders,
            let updatedReminders = self.updateAlarms(forItems: reminders) {
            Controllers.dataModel.update(someReminders: updatedReminders)
    ...
    private func startTimerForNextEventTime() {
        ...
        if  let dataModel = Controllers.dataModel.dataModel {
            ...
            var nextAlarmTime = dataModel.nextAlarmDateForSchedulingTimer
...
extension RCCalendarItem {
    public mutating func stopAlarmButtonClicked() {
        ...
        Controllers.dataModel.update(calendarItem: self)
    }
}
