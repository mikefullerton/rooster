...
extension Schedule {
    public struct Calendars: Equatable, CustomStringConvertible, Loggable {
        ...
        public func calendar(forID id: String) -> ScheduleCalendar? {
            self.calendar(forID: id, inGroups: self.calendarGroups) ??
            self.calendar(forID: id, inGroups: self.delegateCalendarGroups)
        }

        fileprivate func calendar(forID id: String, inGroups groups: [CalendarGroup]) -> ScheduleCalendar? {
            for group in groups {
                if let index = group.calendars.firstIndex(where: { $0.calendar.id == id }) {
