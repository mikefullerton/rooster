...
public struct ScheduleStorageRecord: Equatable, CustomStringConvertible {
    public var calendars: [String: CalendarScheduleItemStorageRecord]
    public var delegateCalendars: [String: CalendarScheduleItemStorageRecord]
    ...
    private static func calendarList(fromCalendarGroups groups: [CalendarGroup]) -> [String: CalendarScheduleItemStorageRecord] {
        var calendarsList: [String: CalendarScheduleItemStorageRecord] = [:]

        groups.forEach {
            $0.calendars.forEach {
                calendarsList[$0.eventKitCalendar.id] = $0.storageRecord
            }
        }
    ...
    private func enabledCalendars(inGroups groups: [String: CalendarScheduleItemStorageRecord]) -> [String] {
...
extension ScheduleStorageRecord: Codable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        self.calendars = try values.decodeIfPresent([String: CalendarScheduleItemStorageRecord].self, forKey: .calendars) ?? [:]
        self.delegateCalendars = try values.decodeIfPresent([String: CalendarScheduleItemStorageRecord].self, forKey: .delegateCalendars) ?? [:]
