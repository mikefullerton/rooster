...
extension ScheduleItem {
    public func openLocationURL() -> Bool {
        if let url = self.knownLocationURL {
            #if os(macOS)
            CoreControllers.shared.locationUtilities.openLocationURL(url) { success, error in
                self.logger.log("""
                    Opened URL: \(url). \
                    Success: \(success), \
    ...
    public func bringLocationAppsToFront() {
        if let url = self.knownLocationURL {
            #if os(macOS)
            CoreControllers.shared.locationUtilities.bringLocationAppsToFront(for: url)
            #endif
