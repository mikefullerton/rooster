...
public class SimpleTimer: CustomStringConvertible, CustomDebugStringConvertible, Loggable {
    ...
    public private(set) var requestedFireCount: Int
    public private(set) var fireCount: Int
    public private(set) var isTiming: Bool
    ...
    private func timerFired(completion: @escaping (_ timer: SimpleTimer) -> Void) {
        ...
        if self.logTimerEvents {
            self.logger.log("""
            timer fired at: \(Date().shortDateAndLongTimeString), \
            \(self.description), will fire again: \(willFireAgain)
            """)
        }
    ...
    public var debugDescription: String {
        ...
        return """
            \(type(of: self)): \
            \(self.name):\(self.id), \
            timer fire date: \(fireDate), \
            interval:\(self.interval), \
            fireCount: \(self.fireCount), \
            requestFireCount: \(self.requestedFireCount), \
            timer: \(String(describing: self.timer )), \
            timer is valid: \(isValid), timer interval: \(interval)
            """
    }
    ...
    public var description: String {
        ...
        return """
            \(type(of: self)): \
            \(self.name):\(self.id), \
            started: \(self.startDate?.shortDateAndLongTimeString ?? "nil"), \
            fire date: \(fireDate), \
            timer fire date: \(timerFireDate)
            """
    }
}
