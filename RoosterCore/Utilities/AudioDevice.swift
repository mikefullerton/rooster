...
class AudioDevices: Loggable {
    ...
    public lazy var allDevices: [AudioDevice] = {
        ...
        let error = AudioObjectGetPropertyData(AudioObjectID(kAudioObjectSystemObject), &propertyAddress, 0, nil, &devicesSize, &devices)
        if error != 0 {
            self.logger.error("deviceName: \(error)")
        }

        return devices.map {
            return AudioDevice(withID: $0)
        }
    }()
    ...
    public static func setOutputDevice(withDeviceID newDeviceID: AudioDeviceID) {
        ...
        let error = AudioObjectSetPropertyData(AudioObjectID(kAudioObjectSystemObject), &propertyAddress, 0, nil, propertySize, &deviceID)
        if error != 0 {
    ...
    static var defaultOutputDevice: AudioDevice {
        ...
        let error = AudioObjectGetPropertyData(AudioObjectID(kAudioObjectSystemObject), &propertyAddress, 0, nil, &propertySize, &deviceID)
        if error != 0 {
            self.logger.error("subDeviceCount: \(error)")
        }
...
public class AudioDevice: Loggable, Identifiable, CustomStringConvertible {
    ...
    public lazy var isAggregateDevice: Bool = {
        return self.transportType == kAudioDeviceTransportTypeAggregate
    }()
...
public extension AudioDevice {
    class Stream: Identifiable, Loggable {
