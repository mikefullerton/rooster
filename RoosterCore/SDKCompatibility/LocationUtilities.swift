...
public class LocationUtilities: NSObject, Loggable {
    ...
    func openURLDirectly(inAppIfPossible url: URL, completion:((_ success: Bool, _ error: Error?) -> Void)?) {
        ...
        if url.absoluteString.contains("webex") {
           // NSWorkspace.shared.open(url)
            // appleinc.webex.com/meet/mfullerton
...
//
    ...
    public func bringLocationAppsToFront(for url: URL) {
        SystemUtilities.bringAnotherApp(toFront: "com.apple.Safari", completion: nil)

        DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + .seconds(3)) {
            if url.absoluteString.contains("webex") {
                SystemUtilities.bringAnotherApp(toFront: "com.webex.meetingmanager", completion: nil)
            }
        }
    }
    ...
    public func openLocationURL(_ url: URL, completion: ((Bool, Error?) -> Void)? = nil) {
        ...
        NSWorkspace.shared.open(url,
            ...
            guard error != nil else {
                self?.logger.error("""
                    opening location URL: \(url), \
                    failed with error: \(error != nil ? error!.localizedDescription : "nil")
                    """)
