...
open class ListViewController<ViewModel: ListViewModel>: SDKViewController,
...
//        view.automaticallyAdjustsContentInsets = false
    ...
    private func defaultWidthInSection(_ section: Int) -> CGFloat {
        guard let viewModel = self.viewModel,
              let tableSection = viewModel.section(forIndex: section) else {
                  self.logger.error("Returning zero for size of item at indexPath: \(section)")
                  return 0
              }

        let contentInsets = self.scrollView.contentInsets
        let layoutInsets = tableSection.layout.insets
    ...
    open func collectionView(_ collectionView: NSCollectionView,
        ...
        let size = CGSize(width: self.defaultWidthInSection(indexPath.section),
    ...
    open func collectionView(_ collectionView: NSCollectionView,
        ...
        let size = CGSize(width: self.defaultWidthInSection(section),
    ...
    open func mouseTrackingCollectionView(_ collectionView: MouseTrackingCollectionView,
        ...
        for item in self.collectionView.visibleItems() where item.highlightState != .none {
            item.highlightState = .none
        }
    }
