...
open class ListViewController: SDKViewController,
    ...
    open func reloadData() {
        ...
        for i in 0..<self.collectionView.numberOfSections {
            for j in 0..<self.collectionView.numberOfItems(inSection: i) {
                ...
                //                if let item = self.collectionView.item(at: IndexPath(item: j, section: i)) {
                //                    print(item)
                //                }
                
                
                
                if let layout = self.collectionView.layoutAttributesForItem(at: indexPath) {
                    print("\(indexPath): \(layout.frame)")
                } else {
                    print("\(indexPath)")
                }
    ...
    @MainActor open func collectionView(_ collectionView: NSCollectionView,
    ...
    @MainActor open func collectionView(_ collectionView: NSCollectionView, numberOfItemsInSection section: Int) -> Int {
        guard let viewModel = self.viewModel,
    ...
    @MainActor open func numberOfSections(in collectionView: NSCollectionView) -> Int {
    ...
    @MainActor private func sizeForItem(_ item: ListViewModelItem) -> CGSize {
        guard let viewModel = self.viewModel else {
