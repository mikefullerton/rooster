...
struct EKEventStoreAuthenticator: Loggable {
    ...
    private func requestAccessToDelegateStore(withUserStore store: EKEventStore,
        ...
        Controllers.appKitPlugin.eventKitHelper.requestPermissionToDelegateCalendars(for: self.store, completion: completion)
    }
    #elseif os(macOS)
    private func requestAccessToDelegateStore(withUserStore store: EKEventStore,
                                              completion: @escaping CompletionBlock) {
    ...
    public func requestAccess(toEventStore eventStore: EKEventStore, completion: @escaping CompletionBlock) {
        ...
        self.requestAccess(toStore: eventStore) { success, error in
            if success {
                ...
                self.logger.error("failed to be granted access to user eventStore: \(eventStore.eventStoreIdentifier) with error: \(error?.localizedDescription ?? "nil")")
