...
class SingleSoundChoiceView : NSView {
    ...
    private lazy var soundPickerButton: NSButton = {
        let button = NSButton.createImageButton(withImage: NSImage(systemName: "square.and.pencil"))
        button.translatesAutoresizingMaskIntoConstraints = false
        button.imageView?.tintColor = Theme(for: self).secondaryLabelColor
        button.setPreferredSymbolConfiguration(NSImage.SymbolConfiguration(pointSize: 18), forImageIn: .normal)
        button.addTarget(self, action: #selector(editSound(_:)), for: .touchUpInside)
        return button
    }()

    private lazy var shuffleButton: NSButton = {
        let button = NSButton.createImageButton(withImage: NSImage(systemName: "shuffle"))
        button.translatesAutoresizingMaskIntoConstraints = false
        button.imageView?.tintColor = Theme(for: self).secondaryLabelColor
        button.setPreferredSymbolConfiguration(NSImage.SymbolConfiguration(pointSize: 16), forImageIn: .normal)
