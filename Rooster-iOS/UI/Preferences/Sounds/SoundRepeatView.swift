...
class SoundRepeatView: LabeledSliderView {
    ...
    init(fixedLabelWidth: CGFloat,
        ...
        self.slider.value = min(self.slider.maximumValue, Float(AppControllers.shared.preferences.soundPreferences.playCount))
        ...
        self.preferencesUpdateHandler.handler = { [weak self] newPrefs, _ in
            ...
            self.slider.value = min(self.slider.maximumValue, Float(newPrefs.soundPreferences.playCount))
            self.updateVolumeSliderImage(withSliderView: self.slider)
        }
    }
