...
class CalendarListCell: UITableViewCell, TableViewRowCell {
    typealias ContentType = EventKitCalendar
    ...
    func viewWillAppear(withData calendar: EventKitCalendar, indexPath: IndexPath) {
        self.calendar = calendar
        self.setCheckBoxTitle(calendar.title)
        self.checkBox.setOn(calendar.isEnabled, animated: false)
    ...
    @objc func checkBoxChecked(_ checkbox: UISwitch) {
        if let calendar = self.calendar {
            calendar.set(subscribed: !calendar.isEnabled)
        }
    }
