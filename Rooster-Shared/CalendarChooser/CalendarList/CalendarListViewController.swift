...
open class CalendarListViewController: ListViewController<CalenderListViewModel>,
                                       CalendarListRowControllerDelegate,
                                       ColorPickerDelegate {
    private var scheduleUpdateHandler = ScheduleUpdateHandler()

    public var minimumContentSize = CGSize(width: 500, height: 100)
    ...
    public func updaterFromColorPicker(_ colorPicker: ColorPicker) -> Updater<CalendarScheduleItem>? {
        (colorPicker.userInfo as? Updater<CalendarScheduleItem>)
    }

    public func calendarFromColorPicker(_ colorPicker: ColorPicker) -> CalendarScheduleItem? {
    ...
    public func colorPickerGetColor(_ colorPicker: ColorPicker) -> SDKColor? {
        if let calendar = self.calendarFromColorPicker(colorPicker) {
            let color = calendar.eventKitCalendar.color
        ...
        if let updater = self.updaterFromColorPicker(colorPicker) {
            updater.update(afterMilliseconds: 200) { [weak self] in
                if var calendar = self?.calendarFromColorPicker(colorPicker) {
                    calendar.eventKitCalendar.cgColor = color.cgColor
