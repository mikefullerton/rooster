...
open class CalendarListRowController: ListViewRowController {
    public private(set) var scheduleItem: CalendarScheduleItem?
    ...
    func updateColors() {
        if  let scheduleItem = self.scheduleItem,
            let calendarColor = scheduleItem.eventKitCalendar.color {
    ...
    override open func willDisplay(withContent content: Any?) {
        guard let scheduleItem = content as? CalendarScheduleItem else {
            return
        }

        self.scheduleItem = scheduleItem
        self.checkBox.title = scheduleItem.eventKitCalendar.title
        ...
        if scheduleItem.eventKitCalendar.isReadOnly {
            icons.append(self.readOnlyIcon)
        }

        if scheduleItem.eventKitCalendar.allowsEvents {
            icons.append(self.eventsIcon)
        }

        if scheduleItem.eventKitCalendar.allowsReminders {
