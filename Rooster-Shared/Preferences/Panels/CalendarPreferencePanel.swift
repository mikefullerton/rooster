    ...
    override public func loadView() {
        ...
        calendarOptionsBox.setContainedViews([
            SinglePreferenceChoiceView(withTitle: "Show Calendar Name on Events and Reminders",
                                       updater: { view in view.checkbox.isOn = Self.prefs.options.contains( .showCalendarName ) },
                                       setter: { view in Self.prefs.options.set(.showCalendarName, to: view.checkbox.isOn) }),
            self.daysPopupMenu
        ])
