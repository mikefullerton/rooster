...
public class StartPlayingSoundAlarmAction: AlarmNotificationAction, SoundDelegate {
    private let startDelayTimer = SimpleTimer(withName: "AlarmNotificationTimer")
    ...
    override public func startAction() {
        if !self.alarmState.contains( .soundsStarted) {
            ...
            let soundPrefs = AppControllers.shared.preferences.preferences(forItemIdentifier: self.itemID).soundPreference
