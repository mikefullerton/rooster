...
//        UInt32(kVK_ANSI_R)
    ...
    public func unregister() {
        if let eventHandlerRef = self.eventHandlerRef {
            let result = RemoveEventHandler(eventHandlerRef)
            self.logger.log("RemoveEventHandler result: \(result)")
            self.eventHandlerBlock = nil
        }

        if let hotKeyRef = self.hotKeyRef {
            let result = UnregisterEventHotKey(hotKeyRef)
            self.logger.log("Unregister hot key result: \(result)")
            self.hotKeyRef = nil
            self.callback = nil
        }
    }
    ...
    public func register(withCallback callback: @escaping Callback) -> Int? {
        ...
        guard status == noErr else {
            self.logger.error("got error from RegisterEventHotKey: \(installStatus)")
            return Int(installStatus)
        }
