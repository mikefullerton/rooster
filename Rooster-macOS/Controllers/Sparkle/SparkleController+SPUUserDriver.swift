    ...
    public func showErrorAlert(_ error: Error) {
        ...
        if nsError.domain == SUSparkleErrorDomain {
            if nsError.code == SUError.noUpdateError.rawValue {
                ...
                self.logger.log("""
                        update check completed ok - no updated needed: \(nsError.localizedDescription): \
                        \(nsError.localizedRecoverySuggestion ?? "")
                        """)
            } else if self.state.contains( [.checking, .userInitiated]) && nsError.code == SUError.appcastError.rawValue {
                self.logger.error("""
                    update check failed, user is not on internal network: \(nsError.localizedDescription): \
                    \(nsError.localizedRecoverySuggestion ?? "")
                    """)
            ...
            alert.alertStyle = NSAlert.Style.critical
            self.logger.error("""
                showUpdateNotFoundWithError: \(nsError.localizedDescription): \
                \(nsError.localizedRecoverySuggestion ?? "")
                """)
        }
...
//        self.hideInfoWindow()
    ...
    public func showUpdaterError(_ error: Error, acknowledgement: @escaping () -> Void) {
        self.logger.error("showUpdaterError: \(String(describing: error))")
        self.state += [ .failed ]
        self.showErrorAlert(error)
